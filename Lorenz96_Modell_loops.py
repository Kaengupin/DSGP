...
if LOOP_T:
    ...
    dt  = np.logspace(-5,-2,nt,endpoint=True)   # vector of different time steps
    TU  = 1                                     # Time unit
...
if INIT:
    ...
    init    = 5000                      # time step of perturbation
...
for init in runs:
    ...
    for t in dt:
        i += 1
        # open files to save
        fX = tables.open_file(f'data/{title1}_{scheme}_Pert_{PERT}_INIT_{init}_TS_{i}.h5', mode='w')
        fY = tables.open_file(f'data/{title2}_{scheme}_Pert_{PERT}_INIT_{init}_TS_{i}.h5', mode='w')
